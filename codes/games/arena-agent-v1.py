def run(game_id: str, agent_name: str = "Xoul에이전트", persona: str = "분석적이고 논리적인 성격."):
    ...
    CONFIG_PATHS = [
        ...
        os.path.join(os.path.dirname(__file__), "..", "config.json"),  # arena_server/../config.json
    ...
    AGENT_NAME = os.environ.get("ARENA_AGENT_NAME", "Xoul에이전트")
    PERSONA = os.environ.get("ARENA_PERSONA", "분석적이고 논리적인 성격. 증거 기반으로 판단.")
    ...
    def main():
        ...
        # 환경변수로 game_id가 지정된 경우 바로 참가
        env_game_id = os.environ.get("ARENA_GAME_ID", "")

        # 1. 대기 중인 게임 찾기 (또는 환경변수에서 지정)
        if env_game_id:
            game_id = env_game_id
            print(f"✅ 환경변수에서 게임 ID 지정: {game_id}")
        else:
            print("🔍 대기 중인 게임 찾는 중...")
            game_id = None
            while not game_id:  # arena-loop
                games = api_get("/arena/games?status=waiting")
                if games.get("games"):
                    ...
                    if countdown is not None:
                        print(f"✅ 대기 게임 발견: {game_id} (참가자 {real_count}명, {countdown}초 후 시작)")
                    else:
                        print(f"✅ 대기 게임 발견: {game_id} (참가자 대기 중)")
                else:
                    time.sleep(3)
                    continue
        ...
        while True:  # arena-loop
            ...
            if action == "speak":
                # 서버가 action=speak으로 보내면 아직 발언 가능한 것
                ...
                if target:
                    api_post(f"/arena/games/{game_id}/vote",
                ...
                if target:
                    api_post(f"/arena/games/{game_id}/night",
